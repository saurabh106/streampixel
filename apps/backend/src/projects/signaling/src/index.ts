...
const configArgsParser = new Command()
  .option(
    '--no_config',
    'Skips the reading of the config file. Only CLI options will be used.',
    false,
  )
  .option(
    '--config_file <path>',
...
program
  .option(
    '--log_folder <path>',
    'Sets the path for the log files.',
    config_file.log_folder || 'logs',
  )
  .addOption(
  ...
  .option(
    '--https',
    'Enables the webserver on https_port and enabling SSL',
    config_file.https || false,
  )
  .addOption(
